...
namespace Memory {
    ...
    function main(): void {
       ...
       // numsPlayer();
        ...
        for (let i: number = 0; i < cardArray.length; i++) {
            ...
            // Karten werden jetzt im HTML angezeigt; (cardArray[i]) = Wert auf Popup-Fenster
        }
   ...
   /* function cardPairs(): void {
    ...
    export function main(): void {
        ...
        let gameInfo: NodeListOf<Element> = document.getElementById( "player" ).getElementsByTagName( "input" );

        for ( let i = 0; i < gameInfo.length; i++ ) {

             if ( ( <HTMLInputElement>gameInfo[i] ).checked ) {
        ...
        for ( let i = 0; i < player.length; i++ ) {
            if ((<HTMLInputElement>player[i] ).value != "" ) {
                playerNames.push(( <HTMLInputElement>player[i] ).value );
            }
        }

        player = document.getElementById( "select" ).getElementsByTagName( "input" );

        if ( currentCardDeck == undefined )
            currentCardDeck = decks["fruechte"];
    ...
    export function onInputEvent( value: number ) {
        ...
        document.getElementById( "cardPairsLabel" ).innerText = value.toString();
        numPairs = value;
    }
    ...
    function repopulateCardForm() {
        ...
        let cardPairsElement = <HTMLInputElement>document.getElementById( "cardPairs" );

        cardPairsElement.max = currentCardDeck.content.length.toString();

        let maxWert = currentCardDeck.content.length;

        let momentanerWert = parseInt( cardPairsElement.value );

        if ( maxWert < momentanerWert ) {
            cardPairsElement.value = maxWert.toString()
        }

        document.getElementById( "cardPairs" ).innerText = kartenPaareElement.value;
    }
    ...
    export function bearbeiteSpielerZahlKlick( element: HTMLInputElement ) {
        ...
        for ( let i = 1; i <= 4; i++ ) {
            ...
            if ( i <= spielerZahl ) {
                ...
                labelElement.style.opacity = "0.0";
                inputElement.value = "";
            }
        }
    }
